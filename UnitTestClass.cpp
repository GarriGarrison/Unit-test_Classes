...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        ...
        for (const auto& [key, value_set] : synonyms_) {
            if (key == first_word)
                for (const auto& value : value_set) {
                    if (value == second_word)
                        return true;
                }
...
void TestAreSynonyms() {
    ...
    assert(synonyms.AreSynonyms("tune", "music") == true);
    assert(synonyms.AreSynonyms("tune", "melody") == false);

    assert(synonyms.AreSynonyms("tunes", "melody") == false);
    assert(synonyms.AreSynonyms("tune", "melodyw") == false);
    assert(synonyms.AreSynonyms("tune", "") == false);
    assert(synonyms.AreSynonyms("", "melody") == false);
    assert(synonyms.AreSynonyms("", "") == false);
}
